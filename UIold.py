...
Dilation là một phép toán hình thái học (morphological operation) được sử dụng để mở rộng các vùng sáng (foreground) trong ảnh nhị phân. Phép toán này làm cho các đối tượng sáng trong ảnh trở nên lớn hơn và có thể kết nối các vùng sáng gần nhau.
    ...
    for c in contours:
        ...
        ratio = w / h
        if (len(approx) == 4 and (
                w - h) > 150 and w > 30 and h > 20 and 20 < x and y > 20):  # and <450 and 200<w<600 and 10<h<300 and 20<y<700) điều kiện để lọc đường viền chỉ còn lại khung số
    ...
    if count == 0:
        ...
        def noise_removal(image):
            ...
            image = cv2.morphologyEx(image, cv2.MORPH_CLOSE,
                                     kernel)  # Phép đóng là sự kết hợp của giãn nở và xói mòn. Nó giúp lấp đầy các lỗ nhỏ trong các vùng sáng của ảnh.
            image = cv2.medianBlur(image,
                                   3)  # làm mờ ảnh bằng cách sử dụng bộ lọc trung vị. Đây là một kỹ thuật xử lý ảnh phổ biến để giảm nhiễu trong ảnh.
            return (image)
        ...
        def thin_font(image):
            ...
            image = cv2.erode(image, kernel,
                              iterations=1)  # Quá trình erode sẽ làm giảm kích thước của các vùng màu trắng trong hình ảnh.
            image = cv2.bitwise_not(image)  # Đảo ngược màu hình ảnh
            return (image)
        ...
        canny = cv2.Canny(img_blur, 100,
                          255)  # thực hiện phát hiện biên cạnh (edge detection) trên ảnh img_blur với ngưỡng dưới là 100 và ngưỡng trên là 255.
        ...
        for ind, cnt in enumerate(cont):
            ...
            if (0.0039 * roiarea < char_area < 0.008 * roiarea) and w < h and (
                    0.25 < ratiochar < 1.5):  # and 50<h<300 and 30<w<150
        ...
        for screenCnt in screenCnt:
            ...
            for ind, cnt in enumerate(cont):
                ...
                if (Min_char * roiarea < char_area < Max_char * roiarea) and (
                        0.26 < ratiochar < 1.5) and w < h and 0.55 > perheight > 0.3:  # and perheight>0.5 and 70<h<260 and 30<w<150
                    if x in char_x:  # Sử dụng để dù cho trùng x vẫn vẽ được
                        x = x + 1
                    char_x.append(x)
                    char_x_ind[x] = ind
