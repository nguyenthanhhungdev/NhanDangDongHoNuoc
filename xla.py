...
class Tab1(Frame):
    def image_process(self, img):
        ...
        if not contours:
            ...
            number_water = self.preprocess_and_segment_characters(
                cropped_img, knn_model, imgThreshplate
            )
        else:
            number_water = self.preprocess_and_segment_characters(
                img, knn_model, contours
            )
    ...
    def preprocess_and_segment_characters(
        self, img, knn_model, contours=None, imgThreshplate=None
    ):
        ...
        if contours:
            for screenCnt in contours:
                roi, imgThresh = self.crop_and_align_image(
                    img, screenCnt, imgThreshplate
                )
                cont, thre_mor = self.find_characters(imgThresh)
                number_water = self.recognize_characters(
                    roi,
                    cont,
                    knn_model,
                    RESIZED_IMAGE_WIDTH,
    ...
    def recognize_characters(
        self, roi, cont, knn_model, RESIZED_IMAGE_WIDTH, RESIZED_IMAGE_HEIGHT, thre_mor
    ):
        char_x_ind = {}
        char_x = []
    ...
    def open_camera(self):
        ...
        while True:
            ...
            if k == ord("s"):
                ...
                cv2.imwrite(
                    "/home/nguyenthanhhung/Downloads/PYTHON/Python/QLTTN/QLTTN/data/image/"
                    + img_name,
                    self.frame,
                )
